...
function finite_differences()
    ...
    @time autodiff(Enzyme.ReverseWithPrimal, not_checkpointed_integration, Const, Duplicated(S, dS), Const(revolve))
    ...
    _ = ShallowWaters.time_integration(S_outer)
    ...
    for s in  steps
        ...
        _ = ShallowWaters.time_integration(S_inner)

        push!(diffs2, (S_inner.Prog.η[25, 25] - S_outer.Prog.η[25, 25]) / s)
